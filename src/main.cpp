...
void TaskComm(void* pvParameters) {
    for (;;) {
        Serial.println("Sending status to CoAP server...");
...
void setup() {
    ...
    xTaskCreatePinnedToCore(TaskHome, "TaskHome", 2048, NULL, 1, NULL, 1);

    // Tạo task Communication trên Core 1
    xTaskCreatePinnedToCore(TaskComm, "TaskComm", 2048, NULL, 1, NULL, 1);
}

void loop() {
    // Không cần code gì ở loop, các task chạy song song
